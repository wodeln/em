...
class basic_exam
{
     ...
     * @param $userid 用户ID
     */
	public function addExamUser($examUser,$basicid,$time){
	    $sql['sql'] = "DELETE FROM x2_user_basic WHERE basicid=".$basicid;
	    $sql['v']   = null;
        $this->db->exec($sql);
        $sql['sql']	= "DELETE FROM x2_openbasics WHERE obbasicid=".$basicid;
        $this->db->exec($sql);
        $timeNow = TIME;
        $timeEnd = $timeNow+$time;
        foreach ($examUser as $v) {
            $sql['sql'] = "INSERT INTO x2_user_basic (basicid,userid) VALUE ($basicid,$v)";
            $this->db->exec($sql);
            $sql['sql'] = "INSERT INTO x2_openbasics (obuserid,obbasicid,obtime,obendtime) VALUE ($v,$basicid,$timeNow,$timeEnd)";
            $this->db->exec($sql);
        }
        return true;
	}
     ...
     * @param $basicid 考场ID
     */
	public function getUserListByBasicid($basicid){
        $sql['sql'] = "SELECT * FROM x2_user_basic AS ub
                      LEFT JOIN x2_user AS u ON ub.userid=u.userid
                      WHERE ub.basicid=".$basicid;
        $sql['v']   = null;
     ...
     * @param $userId 用户ID
     */
    public function getSubjectListByUserId($userId){
		$sql['sql'] = "SELECT * FROM x2_user AS u 
						LEFT JOIN x2_user_class AS uc ON u.userclassid=classid
						WHERE userid=".$userId;
		$sql['v']	= null;
		return $this->db->fetchAll($sql);
	}
	...
	public function getSubjectStr(){
		$sql['sql'] = "SELECT fieldvalues FROM x2_module_fields WHERE field='teacher_subjects'";
		$sql['v']	= null;
		return $this->db->fetch($sql);
	}
