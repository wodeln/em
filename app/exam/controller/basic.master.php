...
class action extends app
{
    ...
    private function addsubject()
    {
        if ($this->ev->get('insertsubject')) {
            ...
            $subjectId=$this->basic->addSubject($args);
            $message = array(
                'statusCode' => 200,
                "message" => "操作成功",
                "callbackType" => "forward",
            ...
            $subjectStr=$this->basic->getSubjectStr();
            $subjectStr=$subjectStr["fieldvalues"]."\r\n".$args["subject"]."=".$subjectId;
            $this->basic->updateSubjectStr($subjectStr);
            $this->G->R($message);
        } else {
            $subjects = $this->basic->getSubjectList();
    ...
    private function modifysubject()
    {
        if ($this->ev->get('modifysubject')) {
            ...
            $subjectid = $this->ev->get('subjectid');
            $subject = $this->basic->getSubjectById($subjectid);
            $searchStr = $subject["subject"]."=".$subjectid;
            $replaceStr = $args["subject"]."=".$subjectid;
            $subjectStr=$this->basic->getSubjectStr();
            $subjectStr = str_replace($searchStr,$replaceStr,$subjectStr["fieldvalues"]);
            $this->basic->updateSubjectStr($subjectStr);
    ...
    private function delsubject()
    {
        ...
        if ($section)
            ...
            $subject = $this->basic->getSubjectById($subjectid);
            $searchStr = $subject["subject"]."=".$subjectid;
            $subjectStr=str_replace(PHP_EOL.$searchStr,"",$subjectStr["fieldvalues"]);
            $this->basic->updateSubjectStr($subjectStr);
        }
        $this->G->R($message);
    }
    ...
    private function modifybasic()
    {
        ...
        if ($this->ev->get('modifybasic')) {
            ...
            $time=365*24*3600;
            if (count($examUser) > 0) {
                $this->basic->addExamUser($examUser, $basicid, $time);
            }
            $message = array(
                'statusCode' => 200,
                "message" => "操作成功",
        ...
        if ($this->ev->get('insertbasic')) {
           ...
           */
            ...
            $message = array(
                ...
                "message" => "操作成功",
                "callbackType" => "forward",
                "forwardUrl" => "index.php?exam-master-basic-setexamrange&basicid={$basicid}&page={$page}{$u}"
            );
            $this->G->R($message);
        } else {
            $subjects = $this->basic->getSubjectList();
    ...
    private function index()
    {
       ...
       /* if (!$search)
            ...
            $args = array();*/
        $args[] = array("AND", "basicid != :basicid", 'basicid', 6);
