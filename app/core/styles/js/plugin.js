...
function markSelectedQuestions(n, o) {
    $("[name='" + n + "']").each(function () {
        if ($('#' + o).val().indexOf(',' + $(this).val() + ',') >= 0) $(this).attr('checked', true);
    });
}
...
function xvars(x) {
    ...
    var core = function (exp, x) {
        ...
        if (x.attr('type') != 'password' && x.val()) {
            x.val(x.val().replace(/^\s+/i, '').replace(/\s+$/i, ''));
        }
        ...
        if (x.attr('type') == 'checkbox') {
            ...
            // if(!needle && (x.val() == '' || x.val()== null))return {result:true};
        }
...
function autocombox() {
    ...
    $.get(url + '&rand=' + Math.random(), function (dt) {
        if (dt.statusCode == '200') {
            var d = dt.html;
            $('select[rel="' + $(_this).attr('name') + '_auto_rel"]').each(function () {
                if ($(this).attr('step') && $(this).attr('step') != '' && parseInt($(this).attr('step')) > step) $(this).remove();
            });
...
function formsubmit() {
    ...
    $(":input", _this).not('.ckeditor').not('#undo_redo').not('#undo_redo_to').each(function () {
    ...
    submitAjax({
        ...
        "query": query,
        "target": target,
        'action-before': $(_this).attr('action-before')
    });
    return false;
}
...
function confirmDialog() {
    ...
    $(_this).on('click', function () {
        $.zoombox.show('confirm', _this, href);
    });
}
...
$(function () {
    ...
    $('body').delegate('.listimgselector', 'dblclick', function () {
        $(this).remove();
    });
});
